...
$results += Invoke-CcodTest 'installer publishes the exact CodexRemote-fix 2.2.2 release artifacts' {
    $package = Get-Content -LiteralPath (Join-Path $repositoryRoot 'package.json') -Raw | ConvertFrom-Json
    Assert-CcodEqual '2.2.2' ([string]$package.version) 'package version is exactly 2.2.2'
    ...
    Assert-CcodEqual 'CodexRemote-fix-2.2.2-setup.exe' $setupName 'Inno output resolves to the exact public setup filename'
    ...
    Assert-CcodEqual 'CodexRemote-fix-2.2.2-setup.exe.sha256.txt' $checksumName 'build script resolves to the exact public checksum filename'
}
...
$results += Invoke-CcodTest 'README and release workflow publish current installer-first branding' {
    ...
    Assert-CcodTrue ($quickStart -cmatch 'CodexRemote-fix-2\.2\.2-setup\.exe') 'English Quick Start names the exact setup artifact'
    Assert-CcodTrue ($quickStart -cmatch 'CodexRemote-fix-2\.2\.2-setup\.exe\.sha256\.txt') 'English Quick Start names the exact checksum artifact'
    ...
    Assert-CcodTrue ($quickStartChinese -cmatch 'CodexRemote-fix-2\.2\.2-setup\.exe') 'Chinese Quick Start names the exact setup artifact'
    Assert-CcodTrue ($quickStartChinese -cmatch 'CodexRemote-fix-2\.2\.2-setup\.exe\.sha256\.txt') 'Chinese Quick Start names the exact checksum artifact'
