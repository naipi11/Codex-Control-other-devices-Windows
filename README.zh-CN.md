...
1. 从 [Releases](https://github.com/naipi11/Codex-Control-other-devices-Windows/releases) 下载 `CodexRemote-fix-2.4.6-setup.exe` 和 `CodexRemote-fix-2.4.6-setup.exe.sha256.txt`，并核对 SHA-256。
2. 运行 `CodexRemote-fix-2.4.6-setup.exe`，无需管理员权限。
...
为 2.4.6 发布提供可直接运行的 `CodexRemote-fix-2.4.6-setup.exe`
及 `CodexRemote-fix-2.4.6-setup.exe.sha256.txt`。
