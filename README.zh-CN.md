...
1. 从 [Releases](https://github.com/naipi11/Codex-Control-other-devices-Windows/releases) 下载 `CodexRemote-fix-2.4.10-setup.exe` 和 `CodexRemote-fix-2.4.10-setup.exe.sha256.txt`，并核对 SHA-256。
2. 运行 `CodexRemote-fix-2.4.10-setup.exe`，无需管理员权限。
...
已验证：Windows 11 · Codex Desktop `26.803.10989.0` · Node.js `22.23.1`；
...
为 2.4.10 发布提供可直接运行的 `CodexRemote-fix-2.4.10-setup.exe`
及 `CodexRemote-fix-2.4.10-setup.exe.sha256.txt`。
